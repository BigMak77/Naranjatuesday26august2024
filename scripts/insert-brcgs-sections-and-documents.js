...
async function importSectionsAndDocuments() {
  console.log('=== BRCGS Sections and Documents Import ===\n');
  ...
  if (stdError || !standards || standards.length === 0) {
    console.error('BRCGS standard not found');
    process.exit(1);
  }

  const brcgsStandard = standards[0];
  console.log(`✓ Found standard: ${brcgsStandard.name} (${brcgsStandard.id})\n`);
  ...
  const clauseTypeId = docTypes && docTypes.length > 0 ? docTypes[0].id : null;
  if (!clauseTypeId) {
    console.error('Clause document type not found. Run import-brcgs-clauses.js first.');
    process.exit(1);
  }
  console.log(`✓ Found document type: Clause (${clauseTypeId})\n`);
  ...
  console.log('Step 1: Inserting sections...\n');

  // Insert sections
  const sectionsToInsert = clauses.map(clause => ({
    code: clause.code,
    title: clause.title,
  ...
  for (let i = 0; i < sectionsToInsert.length; i += batchSize) {
    ...
    if (error) {
      console.error(`Error inserting section batch ${i / batchSize + 1}:`, error.message);
    } else {
  ...
  console.log(`\n✓ Inserted ${insertedSections} sections\n`);
  console.log('Step 2: Inserting documents...\n');

  // Insert documents
  const documentsToInsert = clauses.map(clause => ({
    title: clause.title,
    reference_code: clause.code,
    notes: clause.summary,
    document_type_id: clauseTypeId,
    section_id: sectionIds[clause.code],
    current_version: 1,
    archived: false,
  ...
  for (let i = 0; i < documentsToInsert.length; i += batchSize) {
    ...
    if (error) {
      console.error(`Error inserting document batch ${i / batchSize + 1}:`, error.message);
    } else {
...
importSectionsAndDocuments().catch(error => {
