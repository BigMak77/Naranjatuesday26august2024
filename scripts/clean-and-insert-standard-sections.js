...
async function main() {
  console.log('Starting standard_sections cleanup and update...\n');
  ...
  console.log(`Using standard: ${standards[0].name} (${standardId})\n`);
  ...
  if (existingSections && existingSections.length > 0) {
    ...
    // Check which ones are referenced by audit_questions
    console.log('Checking for references in audit_questions...');
    const { data: referencedSections } = await supabase
      .from('audit_questions')
      .select('standard_section_id')
      .in('standard_section_id', existingSections.map(s => s.id));

    const referencedIds = new Set(referencedSections?.map(r => r.standard_section_id) || []);

    console.log(`${referencedIds.size} sections are referenced by audit_questions\n`);

    // Delete audit_questions that reference these sections
    if (referencedIds.size > 0) {
      console.log('Deleting audit_questions that reference these sections...');
      const { error: deleteQuestionsError } = await supabase
        .from('audit_questions')
        .delete()
        .in('standard_section_id', Array.from(referencedIds));

      if (deleteQuestionsError) {
        console.error('Error deleting audit_questions:', deleteQuestionsError);
        console.log('\nYou may need to manually clean up the audit_questions table first.');
        console.log('Or run this query: DELETE FROM audit_questions WHERE standard_section_id IN (SELECT id FROM standard_sections WHERE standard_id = \'' + standardId + '\');');
        process.exit(1);
      } else {
        console.log(`Deleted audit_questions successfully.\n`);
      }
    }
