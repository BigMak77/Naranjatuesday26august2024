...
serve(async () => {
  ...
  if (templateErr) {
    return new Response(`Template fetch error: ${templateErr.message}`, { status: 500 })
  }
    ...
    for (const shift of activeShifts) {
      ...
      if (!existing || existing.length === 0) {
        ...
        if (insertErr || !newCheckData) {
          createdLogs.push(`❌ Failed to insert check for ${template.name} – ${shift}`)
          continue
        }
        ...
        if (itemInsertErr) {
          createdLogs.push(`⚠️ Check created but items failed: ${template.name} – ${shift}`)
        } else {
