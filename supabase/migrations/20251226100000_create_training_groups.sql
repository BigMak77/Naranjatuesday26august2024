...
-- Add foreign key constraints for item_id based on item_type
-- Note: We can't add conditional FK constraints directly in PostgreSQL,
-- so we'll handle this through triggers
...
-- Trigger to sync training when a member is added
...
-- Trigger to sync training when an assignment is added
...
-- Trigger to remove training when a member is removed
...
-- Trigger to remove training when an assignment is removed
