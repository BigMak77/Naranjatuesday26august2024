...
export async function syncRoleProfileAssignments(auth_id: string) {
  ...
  const [{ data: modules }, { data: documents }, { data: behaviours }] = await Promise.all([
    supabase.from('role_profile_modules').select('module_id').in('role_profile_id', profileIds),
    supabase.from('role_profile_documents').select('document_id').in('role_profile_id', profileIds),
