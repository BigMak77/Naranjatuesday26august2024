...
export default function AddDocumentPage() {
  ...
  const [documentType, setDocumentType] = useState<'policy' | 'ssow' | 'work_instruction' | ''>('policy')
  ...
  useEffect(() => {
    if (!standardId) { setSections([]); setSectionId(''); return }
    let cancelled = false
    ;(async () => {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (referenceCode.trim()) {
        ...
        if (checkError) { alert('Error checking reference code.'); return }
      ...
      if (!publicUrl) { alert('Failed to get public URL for uploaded file.'); return }
      ...
      if (docError || !newDoc) { alert('Failed to create document.'); return }
      ...
      if (versionError) { alert('Failed to save version 1.'); return }
    ...
    <div className="after-hero">
      <div className="global-content">
        <main className="add-document-main">
          ...
          {!showModuleAttach ? (
            <NeonForm title="Add New Document" onSubmit={handleSubmit} submitLabel={submitting ? 'Saving…' : 'Save Document'}>
              {/* Title */}
              <div className="neon-form-row">
                <label htmlFor="doc-title" className="neon-label">Title *</label>
                <input
                  id="doc-title"
                  type="text"
              ...
              {/* Reference Code */}
              <div className="neon-form-row">
                <label htmlFor="doc-ref" className="neon-label">Reference Code</label>
                <input
                  id="doc-ref"
                  type="text"
                  className="neon-input"
              ...
              {/* Document Type */}
              <div className="neon-form-row">
                <label htmlFor="doc-type" className="neon-label">Document Type *</label>
                <select
                  id="doc-type"
                  value={documentType}
              ...
              <div className="neon-form-row">
                <div className="neon-form-col">
                  <label htmlFor="doc-standard" className="neon-label">Standard *</label>
                  <select
                    id="doc-standard"
                    value={standardId}
                    onChange={e => { setStandardId(e.target.value); setSectionId('') }}
                    className="neon-input"
                    required
                  >
                ...
                <div className="neon-form-col">
                  <label htmlFor="doc-section" className="neon-label">Section</label>
                  <select
                    id="doc-section"
                    value={sectionId}
              ...
              {/* File Upload */}
              <div className="neon-form-row">
                <label htmlFor="doc-file" className="neon-label">Upload File (PDF) *</label>
                <input
                  id="doc-file"
                  type="file"
              ...
              {/* Version Notes */}
              <div className="neon-form-row">
                <label htmlFor="doc-notes" className="neon-label">Version Notes</label>
                <textarea
                  id="doc-notes"
                  className="neon-input"
                  rows={3}
            ...
            <Modal open={showModuleAttach} onClose={() => { setShowModuleAttach(false); setShowModuleSelector(false); router.push('/admin/documents'); }}>
              <NeonForm title="Attach Document to Training Module" onSubmit={e => e.preventDefault()} submitLabel="Finish">
                <h2 className="neon-form-title">Would you like to attach this document to a training module?</h2>
                ...
                {showModuleSelector && (
                  !addingNewModule ? (
                    <div className="mt-6">
                      <label className="neon-label">Select Existing Module</label>
                      <select
                        className="neon-input"
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            const { data: module, error } = await supabase
                              ...
                              .select()
                              .single()
                            if (error || !module) { alert('Failed to create module.'); return }
