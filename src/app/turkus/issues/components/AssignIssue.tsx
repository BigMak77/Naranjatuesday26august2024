...
export default function AssignIssue({ issueId }: { issueId: string }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (departmentId) {
        supabase.from('users').select('id, first_name, last_name, department_id').eq('department_id', departmentId).then(({ data }) => setUsers(data || []));
      } else {
        setUsers([]);
      }
    });
  ...
  return (
    <div className="centered-content">
      <NeonForm title="Assign Issue" onSubmit={handleAssign}>
        ...
        <button type="submit" className="neon-btn w-full mt-4" disabled={loading}>
