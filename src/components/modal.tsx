...
export default function Modal({ open, title, onClose, initialFocusRef, children }: ModalProps) {
  ...
  useEffect(() => {
    ...
    const target = initialFocusRef?.current || dialogRef.current?.querySelector<HTMLElement>('[data-autofocus]') || dialogRef.current?.querySelector<HTMLElement>('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])') || null;
    ...
    <>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
          {title ? <h2 id="app-modal-title" className="neon-form-title">{title}</h2> : <span />}
          <button className="neon-btn neon-btn-close" aria-label="Close" onClick={onClose}>×</button>
        </div>
        <div>{children}</div>
      </div>
    </>
