...
export default function AddTrainerWidget({
  onAdded,
}: {
  onAdded?: () => void;
}) {
  ...
  // Fetch users for selected department (not archived, not already trainer)
  useEffect(() => {
    setUsers([]);
    setSelectedUsers([]);
    ...
    supabase
      .from("users")
      .select("id, first_name, last_name, is_trainer")
      ...
      .then(({ data, error }) => {
        ...
        else setUsers((data || []).filter(u => !u.is_trainer));
      });
  }, [selectedDept]);
  ...
  return (
    <form
      ...
      style={{ maxWidth: 400 }}
    >
      <h2 className="neon-section-title">Mark Users as Trainers</h2>
      ...
      {success && <p className="neon-success">Users marked as trainers!</p>}
      <button
        type="submit"
