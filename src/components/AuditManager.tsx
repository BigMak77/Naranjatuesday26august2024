...
import { FiPlus, FiClipboard, FiSend, FiFileText, FiHelpCircle, FiShield } from 'react-icons/fi';
...
export const useAuditManager = () => {
  ...
  if (!ctx) throw new Error('useAuditManager must be used inside <AuditManager/>');
  return ctx;
}
...
export default function AuditManager() {
  ...
  const tabs = useMemo(() => ([
    ...
    { key: 'auditors',    label: 'Auditors',         icon: <FiShield className="folder-tab-icon" /> },
  ] satisfies { key: TabKey; label: string; icon: React.ReactNode }[]), []);

  return (
    <Ctx.Provider value={{ activeTab, setActiveTab: handleTabChange, refreshKey, bumpRefresh }}>
      <div className="audit-manager-content">
        <div className="audit-manager-inner">
          ...
          <div className="flex gap-3 mb-3">
            ...
            {statsError && (
              <div className="text-xs text-red-400 self-center">Stats error: {statsError}</div>
            )}
          </div>
