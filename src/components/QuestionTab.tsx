...
export default function QuestionTab() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [qRes, dRes] = await Promise.all([
      supabase.from('audit_questions').select('*').order('question_text', { ascending: true }),
  ...
  const addQuestion = async () => {
    ...
    if (data) {
      setQuestions(prev =>
        [...prev, data as Question].sort((a, b) => (a.question_text || '').localeCompare(b.question_text || ''))
      );
    }
  };
  ...
  const bulkAddQuestions = async (bulkText: string) => {
    ...
    const rows = lines.map(question_text => ({ question_text, fail_department_id: null }));
